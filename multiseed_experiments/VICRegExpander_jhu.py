...
def load_tabula_muris_data():
    ...
    tm_dataset_path = data_dir / "pickled" / "tabula_muris" / f"tm_dataset_jhu.pkl"
    tm_dataloader_path = data_dir / "pickled" / "tabula_muris" / f"tm_dataloader_jhu.pkl"
    tm_adata_train_path = data_dir / "pickled" / "tabula_muris" / f"tm_adata_jhu.pkl"
    tm_adata_test_path = data_dir / "pickled" / "tabula_muris" / f"tm_adata_jhu.pkl" # assuming same..?
...
if __name__ == "__main__":
    ...
    precomputed_gene_clusters_path =  precomputed_dir / f'tm_dataset_jhu_precomputed_gene_clusters.pkl'
    precomputed_mu_sigma_path = precomputed_dir / f'tm_dataset_jhu_precomputed_mu_sigma.pkl'
    ...
    experiment_name = f"genes{PARAMETERS['num_genes']}_batch{PARAMETERS['batch_size']}_latent{PARAMETERS['latent_dimension']}_seed{SEED}"
    augmentations_used_str = '_'.join(augmentations_used)
    print(f'{augmentations_used_str=}')

    best_trial_results_checkpoints_path = f'best_trial_results/{VERSION}/checkpoints/multiseed_jhu'
    ...
    # 5) Save best results
    best_trial_results_path = f'best_trial_results/jhu'
    ...
    output_folder = f"figures/jhu/multiseed_normalized/{experiment_name}_{augmentations_used_str}_epoch={saved_epoch}_final-loss={saved_loss:.4f}"
    ...
    # UMAP Skin
    fig = sc.pl.umap(
        tm_adata_test[tm_adata_test.obs['tissue'] == 'bone marrow'].copy(), color='Celltype', show=False, return_fig=True
    )
    ...
    # UMAP Limb_Muscle
    fig = sc.pl.umap(
        tm_adata_test[tm_adata_test.obs['tissue'] == 'limb muscle'].copy(), color='Celltype', show=False, return_fig=True
    )
