...
# -------------------------
# 8. Define an Optuna Objective
# -------------------------
def objective(trial: optuna.Trial) -> float:
    """
    1) Sample hyperparameters from Optuna.
    2) Create and train the VICReg model for a few epochs.
    3) Return the final validation loss to be minimized.
    """
    

    # Hyperparameter search space
    # sim_weight = trial.suggest_float("sim_weight", 1.0, 100.0, log=True)
    # var_weight = trial.suggest_float("var_weight", 1.0, 100.0, log=True)
    # cov_weight = trial.suggest_float("cov_weight", 0.1, 5.0, log=True)
    dropout_rate_DO = trial.suggest_float("dropout_rate_DO", 0.1, 0.8, step=0.05)
    dropout_rate_gSS = trial.suggest_float("dropout_rate_gSS", 0.1, 0.8, step=0.05)
    sigma_fill = trial.suggest_float('sigma_fill', 0.1, 10, step=0.05)

    # Build model
    model = scRNASeqE_VICRegExpander(
        PARAMETERS,
        ...
        var_weight=25.0,
        cov_weight=1.0,
        dropout_rate_DO=dropout_rate_DO,
        dropout_rate_gSS=dropout_rate_gSS,
        sigma_fill=sigma_fill,
        std_target=1.0,
        eps=1e-4,
    ...
    trainer = Trainer(
        max_epochs=PARAMETERS['trial_epochs'],
        accelerator="gpu",
        # devices=[gpu_index],  # specify which GPU to use
        devices=-1,
        strategy='ddp',
    ...
    # We retrieve the last logged validation loss from trainer.callback_metrics
    # (make sure val_loss_vicreg is actually computed in validation_step)
    final_val_loss = trainer.callback_metrics["val_loss_vicreg"].item()
...
if __name__ == "__main__":
    ...
    study.optimize(objective, n_trials=PARAMETERS['n_trials'], n_jobs=1)  # number of trials

    # 4) Train best model

    print('Now training best model')
    best_trial = study.best_trial
    # best_sim_weight = best_trial.params["sim_weight"]
    # best_var_weight = best_trial.params["var_weight"]
    # best_cov_weight = best_trial.params["cov_weight"]
    best_dropout_rate_DO = best_trial.params["dropout_rate_DO"]
    best_dropout_rate_gSS = best_trial.params["dropout_rate_gSS"]
    best_sigma_fill = best_trial.params['sigma_fill']
    ...
    final_model = scRNASeqE_VICRegExpander(
        ...
        std_target=1.0,
        eps=1e-4,
        dropout_rate_DO=best_dropout_rate_DO,
        dropout_rate_gSS=best_dropout_rate_gSS,
        sigma_fill=best_sigma_fill,
        augmentations_pipeline=augmentations_pipeline
    )
