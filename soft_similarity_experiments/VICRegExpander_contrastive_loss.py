...
similarity_dir = repo_dir / "similarity_matrix"
...
if __name__ == "__main__":
    ...
    experiment_name = f"genes{PARAMETERS['num_genes']}_batch{PARAMETERS['batch_size']}_latent{PARAMETERS['latent_dimension']}_fixedloss_seed{SEED}"
    augmentations_used_str = '_'.join(augmentations_used)
    print(f'{augmentations_used_str=}')

    best_trial_results_checkpoints_path = f'best_trial_results/{VERSION}/checkpoints/multiseed/'
    os.makedirs(best_trial_results_checkpoints_path, exist_ok=True)

    early_stop = pl.callbacks.EarlyStopping('val_loss_similarity', patience=10)
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        monitor='val_loss_similarity',
        filename=(f'{experiment_name}_'
                  f'{augmentations_used_str}_'
                  'epoch={epoch}-val_loss={val_loss_similarity:.4f}'),
